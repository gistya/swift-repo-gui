...
enum ActionHelp {
    ...
    static let catalog: [String: ActionHelpDescriptor] = [
        ...
        "action.updateDependencies": ActionHelpDescriptor(
            ...
            summary: "Runs update-checkout with --skip-repository swift so your current swift branch is never moved, syncing the sibling repos and refreshing the project. A --scheme is added only when you pick one in the Checkout scheme menu; --match-timestamp only when you tick the Match timestamp checkbox (off by default).",
            practicalAdvice: "Run this after checking out a different swift branch/PR, or when a fresh pull moved the llvm-project pin and CI failed on an ABI/API mismatch you can't reproduce locally. --match-timestamp is the whole point: instead of fast-forwarding each sibling to its branch tip (which pairs your swift with an even-newer LLVM whose API the compiler doesn't yet call correctly, breaking the build), it checks out each sibling at the last commit dated at or before your swift HEAD's commit date, reconstructing the exact llvm-project/cmark/swift-syntax that swift commit was written against. By default no --scheme is passed and the swift repo is skipped (--skip-repository swift), so your current branch — including a personal/fork branch that is not a configured scheme — is never disturbed; update-checkout uses its own default scheme for the siblings and updates them to their latest. Tick Match timestamp to instead pin each sibling to its commit at your swift HEAD's date (--match-timestamp). Pick a specific scheme in the Checkout scheme menu only when you want the siblings tracked to a known branch scheme (e.g. release/6.x). Don't run it when you haven't switched branches and the tree already builds — it just re-resolves the siblings to where they already are, and can surprise you by moving LLVM out from under a good build. Prefer Update & Rebuild Changed when you also want the changed repos rebuilt in one step."
        ),
        ...
        "action.updateAndClean": ActionHelpDescriptor(
            id: "action.updateAndClean",
            title: "Update & Clean Tree",
            summary: "Runs the same update-checkout as Update Dependencies (--skip-repository swift, so your swift branch is left untouched; --match-timestamp only if that checkbox is ticked), then deletes the active build sub-directory (build/<subdir>) outright. It does not start a build.",
            practicalAdvice: "The maintenance button for a poisoned tree: reach for it when an existing build directory was configured against a different toolchain or a stale CMake cache (a host-compiler change, an Xcode upgrade, a swiftly-vs-Xcode mismatch) and you want the next build to reconfigure from a clean slate on freshly-synced sources. It wipes only the sub-directory the current flow uses — the Build tab targets your active/detected subdir, the Toolchain tab targets buildbot_osx — so other bot-mode trees are left intact. Unlike Fresh Rebuild it deletes and stops rather than deleting-then-rebuilding, so you stay in control of when the multi-hour build starts; unlike Fresh Dependency it removes the whole configured tree, not one repo's ninja artifacts. Gotcha: because the delete is total, the following build is a from-scratch compile."
        ),
